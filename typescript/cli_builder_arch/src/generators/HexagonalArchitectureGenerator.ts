...
export class HexagonalArchitectureGenerator extends ArchitectureGenerator {
  protected async generateArchitecture(): Promise<void> {
    ...
    await this.createDirectory('src/infra/adapters');
    await this.createDirectory('src/infra/service');
    ...
    await this.createDirectory('src/app/dto');
    ...
    await this.createFile('src/infra/adapters/example.adapter.ts', this.getAdapter());
    await this.createFile('src/infra/service/example.service.ts', this.getInfraService());
    ...
    await this.createFile('src/app/dto/example.dto.ts', this.getDto());
...
import { IExampleUseCase } from '../../domain/interfaces/example.interface';
...
describe('Hexagonal Architecture Lambda Handler', () => {
...
import { ExampleAdapter } from './infra/adapters/example.adapter';
import { ExampleUseCase } from './app/useCase/example.use-case';
import { ExampleInfraService } from './infra/service/example.service';
